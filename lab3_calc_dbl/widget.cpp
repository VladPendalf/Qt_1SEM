...
void Widget::on_resbutton_clicked() //вычисление результата
{
    ...
    if(ui->div->isChecked())//блок деления
    {
        ...
        if (ok)
        {
            ...
            if (ok)
            {
                ...
                {
                    if ( fabs(op2) < __DBL_MIN__) //если модуль числа меньше минимального считываемого значения DBL, то это деление на 0
                    {
    ...
    if(ui->sqrt->isChecked())
    {
        ...
        if(ok && (op1 >= __DBL_MIN__))//проверка флага преобразования и числа ( на положительность)
        {
            //DBL_MIN = 2.2250738585072014e-308
            res = sqrt(op1);
            if (!qIsInf(res)) //Если "res" не равно бесконечности  и не равен числу NaN
