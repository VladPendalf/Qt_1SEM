    ...
    , ui(new Ui::MainWindow)
{
    ...
    ui->spinBox_column->setMaximum(200);
...
void MainWindow::on_Button_random_clicked()//кнопка рандом
{
    ...
    for (int i = 0; i < ui->tableWidget->rowCount(); i++)
    {
        for (int j = 0; j < ui->tableWidget->columnCount();j++)
        {
            ...
            {
                ui->tableWidget->item(i,j)->setText(QString::number(rand()%100-60));//рандом + запись
            ...
            else//если не существует
            {
                ...
                ui->tableWidget->item(i,j)->setText(QString::number(rand()%100-60));
...
void MainWindow::on_Button_sort_clicked()//кнопка сортировки
{
    ...
    {
        int i = 1, row_count = ui->tableWidget->rowCount(), column_count = ui->tableWidget->columnCount();
        while (size > 200)
        {
...
void MainWindow::on_Button_search_clicked()
{
    if (no_auto_change)
    {
        ...
        if (gl && loc)
        {
            ...
            {
                Bin_Sort(arr, ui->lineEdit_search->text().toDouble(), size);//бинарный поиск <3
            }
            else // если нет
            {
